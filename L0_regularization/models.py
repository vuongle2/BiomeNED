...
from L0_regularization.l0_layers import L0Conv2d, L0Dense
from L0_regularization.base_layers import MAPConv2d, MAPDense
from L0_regularization.utils import get_flat_fts
...
class L0MLP(nn.Module):
    def __init__(self, input_dim, num_classes, layer_dims=(300, 100), N=50000, beta_ema=0.999,
        ...
        for i, dimh in enumerate(self.layer_dims):
            ...
            lamba = lambas[i] if len(lambas) > 1 else lambas[0]
        ...
        if len(self.layer_dims) >0:
            last_hidden_dim = self.layer_dims[-1]
        else: # for the case of no hidden
            last_hidden_dim = input_dim

        layers.append(L0Dense(last_hidden_dim, num_classes, droprate_init=0.5, weight_decay=self.weight_decay,
